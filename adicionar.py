...
def set_default_email():
    with app.app_context():
        try:
            # Set the default email for all Medico records
            default_email = "t3st.dummy2077@gmail.com"
            medicos = Medico.query.all()
            for medico in medicos:
                medico.email = default_email
                print(f"Email atualizado para {medico.nome}: {medico.email}")

            db.session.commit()
            print("Todos os e-mails foram atualizados com sucesso.")
        
        except Exception as e:
            print(f"Erro ao atualizar e-mails: {e}")
            db.session.rollback()
